...
from termutils.config.styles import styles as styles_dict
from termutils.obj.Color import Color

class String(UserString):

    def __init__(
    self, string:str, foreground:Union[Color,str] = None,
    background:Union[Color,str] = None, style:str = None):
        '''
            Creates an instance of class `String`.

            Argument `color` should be a known color in /config/rgb.py or an
            instance of <class 'Color'>.

            Argument `style` should be a known style in /config/styles.py.
        '''
    ...
    def set_foreground(self, color:Union[Color,str] = None):
        '''
        ...
        if color is None:
            ...
            if not isinstance(color, Color):
                if not Color._is_color(color):
                    msg = (
                        f'\n\nAttempt to pass unknown color `{color}` to  '
                        f"argument `color` in `set_foreground` for <class "
                        f"'String'>. Use a known color (see classmethod "
                        f"Color.list_colors()) or an instance of <class "
                        f"'Color'>.\n"
                    )
                    raise ValueError(msg)
                color = Color.palette(color)
            temp = (
                f'{esc};{color._rgb[0]:d};{color._rgb[1]:d};'
                f'{color._rgb[2]:d}'
            )
        self._fore = color
        self._fore_str = temp

    def set_background(self, color:Union[Color,str] = None):
        '''
        ...
        if color is None:
            ...
            if not isinstance(color, Color):
                if not Color._is_color(color):
                    msg = (
                        f'\n\nAttempt to pass unknown color `{color}` to  '
                        f"argument `color` in `set_background` for <class "
                        f"'String'>. Use a known color (see classmethod "
                        f"Color.list_colors()) or an instance of <class "
                        f"'Color'>.\n"
                    )
    ...
    def set_style(self, style:str):
        ...
        if style is None:
            self._style_str = ''
        elif style not in styles_dict.keys():
            styles_str = ', '.join(styles_dict.keys())
            msg = (
                f'\n\nAttempt to pass unknown key `{style}` to argument '
                f"`style` in constructor for <class 'String'>.  Use one of the "
                f'following styles: {styles_str}.\n'
            )
            raise ValueError(msg)
        else:
            style = style.lower()
            self._style_str = f'{styles_dict[style]};'
        self._style = style

    def __set__(self, string:Union['String',str]) -> None:
        '''
            Changes the current String's text, or replaces the instance
            completely, depending on the given type of argument `string`.
        '''
    ...
    def partition(self, *args, **kwargs):
        ...
        for i in super().partition(*args, **kwargs):
            new_list.append(
                self.__class__(new_str, self._fore, self._back, self._style)
            )
