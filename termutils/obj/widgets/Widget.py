...
class Widget:
    ...
    def __init__(
    self, y0:int, x0:int, y1:int, x1:int, background:Union[str, Color] = None,
    foreground:Union[str, Color] = None, style:str = None):
        '''
        ...
        elif style.lower() not in styles_dict.keys():
            styles_str = ', '.join(styles_dict.keys())
            msg = (
                f'\n\nArgument `style` received an unknown option `{style}` '
                f' in constructor for {self._type}.  Use one of the '
                f'following styles: {styles_str}.\n'
            )
            raise ValueError(msg)

        self._style_fmt = f'{styles_dict[style.lower()]};'
        self._style = style

        self.ANSI_format = (
            f'\033[{self._style_fmt}{self._fore_fmt};{self._back_fmt}m'
        )
    ...
    # GETTERS
    def write(self, ellipsis:bool = False):
        '''
        ...
        for i, row_idx in enumerate(range(y_start, y_stop)):
            ...
            if ellipsis:
                ...
                row = (
                    f'{self.ANSI_format}'
                    f'{self._rows[row_idx][x_start:x_stop]}'
                    f'\033[m'
                )
                print(
                    row, end = '', flush = True
